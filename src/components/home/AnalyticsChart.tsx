...
export default function AnalyticsChart() {
  ...
  if (!mounted) {
    return (
      <div className="flex h-full min-h-[400px] w-full animate-pulse flex-col rounded-3xl border border-slate-200/60 bg-white p-6">
        <div className="mb-6 flex justify-between">
          <div className="h-8 w-32 rounded-xl bg-slate-100"></div>
          <div className="h-8 w-48 rounded-xl bg-slate-100"></div>
        </div>
        <div className="flex-1 rounded-2xl bg-slate-50"></div>
      </div>
    );
  }

  return (
    <div className="flex h-full min-h-[400px] flex-col rounded-3xl border border-slate-200/60 bg-white p-6 shadow-sm">
      <div className="mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-extrabold text-slate-900">Tren Analitik</h2>
          <p className="text-sm font-medium text-slate-500 mt-1">Visualisasi mingguan</p>
        </div>
          ...
          {[40, 65, 45, 80, 55, 90, 75].map((height, i) => (
            <div key={i} className="group relative flex flex-1 flex-col items-center justify-end h-full">
              <div 
                ...
                style={{ height: `${height}%` }}
              ></div>
              <span className="absolute -bottom-7 text-xs font-bold text-slate-500">
